...
def compute_bitmasks(busy_times_list, time_slots):
    ...
    for busy_times in busy_times_list:
        ...
        for slot in time_slots:
            ...
            slot_start = datetime.strptime(slot_start_str, time_format).replace(tzinfo=timezone.utc)
            slot_end = datetime.strptime(slot_end_str, time_format).replace(tzinfo=timezone.utc)

            is_busy = False
            for busy in busy_times:
                busy_start = parser.isoparse(busy["start"]).astimezone(timezone.utc)
                busy_end = parser.isoparse(busy["end"]).astimezone(timezone.utc)
