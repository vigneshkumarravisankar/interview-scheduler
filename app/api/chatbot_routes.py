...
async def execute_query(request: dict):
    ...
    try:
        ...
        # Extract role from payload, handling both "role" and "user_role" fields
        user_role = payload_dict.get("user_role", payload_dict.get("role", "HR"))
        logger.info(f"[API] User role from payload: {user_role}")

        # Normalize role for case-insensitive comparison
        normalized_role = user_role.upper() if user_role.lower() == "hr" else user_role.title()
        logger.info(f"[API] Normalized role: {normalized_role}")
        ...
        if normalized_role not in valid_roles:
            ...
            normalized_role = "HR"  # Default to HR for unrecognized roles
        ...
        try:
            response = ChatbotServiceEnhanced.generate_response(
                ...
                user_role=normalized_role  # Pass the normalized user role
            )
            ...
            try:
                logger.info(f"[API] Calling ChatbotService with message: '{message}' and role: '{normalized_role}'")
                ...
                logger.error(f"[API] Error in standard ChatbotService: {str(service_error)}")
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail=f"Error generating response: {str(service_error)}"
                )
        ...
        logger.info(f"[API] Response user_role: {normalized_role}")

        final_response = {
            "message": response.get("response", ""),
            "sessionId": sessionId,
            "success": True,
            "user_role": normalized_role,  # Use the normalized role in the response
            "result": result
        }
